#!/usr/bin/env python3
"""
Submits a CDSE Batch Processing V2 job for Sentinel-3 OLCI Level 2 Water (s3_wfr).
...
Three stages, each its own subcommand (so each can run as its own Argo step):
    ...
    s3-wfr-cdse-pipeline submit -d 2026-02-09   # create + analyse + start; prints
                                                # {"jobs": [{date, job_id, timeliness, status}]}
    s3-wfr-cdse-pipeline wait --job-id <id>     # poll until terminal; exit 0 iff DONE
    s3-wfr-cdse-pipeline stac -d 2026-02-09     # build + upload STAC items
 
End-to-end example:
 
    DATE=2026-02-09
    OUT=$(s3-wfr-cdse-pipeline submit -d "$DATE" --full-aoi)
    JOB_ID=$(jq -r '.jobs[0].job_id' <<< "$OUT")
    TIMELINESS=$(jq -r '.jobs[0].timeliness' <<< "$OUT")
    s3-wfr-cdse-pipeline wait --job-id "$JOB_ID"
    s3-wfr-cdse-pipeline stac -d "$DATE" --timeliness "$TIMELINESS"
"""
...
S3_OLCI_L2_COLLECTION = "sentinel-3-olci-l2"
...
# critical) <= 1 month. Used here only for the delivery folder token and the
# stac --timeliness choice - the reference s3_olci_l2_wfr item does NOT carry
# product:timeliness / product:timeliness_category properties (unlike LFR),
# so the item builder does not set them.
TIMELINESS_DURATION = {"NR": "PT3H", "ST": "PT48H", "NT": "P1M"}

# Set key order of properties in metadata.json
PROPERTY_ORDER = [
    "title",
    "description",
    "platform",
...
DEAFRICA_S3_BUCKET = "deafrica-sentinel-3-olci-l2-water"
...
def deafrica_base_uri(
    ...
    The DE Africa prefix for a dataset:
        ...
        s3://{bucket}/Sentinel-3/OLCI/OL_2_WFR/{YYYY}/{MM}/
            S3_OL_2_WFR_{YYYYMMDD}_{timeliness}_{tile}
    """
...
# Overrides geopackage input, acting as an AOI filter: only intersecting features are processed.
TEST_BBOX = [13.011000000000001, -25.314, 14.016000000000002, -24.399]
...
def _fetch_grid_from_cloudferro(grid_href: str) -> str:
    ...
    local_path = os.path.join(
        tempfile.gettempdir(), f"s3_wfr_grid_{os.path.basename(key)}"
    )
...
def load_tiling_grid(sat_config: dict) -> gpd.GeoDataFrame:
    """
    Load the s3_wfr tiling geopackage into a single EPSG:4326 GeoDataFrame.
    ...
    log.info(f"Loading s3_wfr tiling grid from {grid_href} ...")
...
def _acquisition_key(product_id: str) -> tuple[str, str]:
    ...
    S3A_OL_2_WFR____20160407T111839_...  ->  ("S3A", "20160407T111839").
    """
    return product_id[:3], product_id[16:31]


def fetch_s3_wfr_scenes(session, bbox: list[float], date: str) -> list[dict]:
    """
    Query the SH catalog for all OL_2_WFR scenes
    ...
    """
    body = {
        "collections": [S3_OLCI_L2_COLLECTION],
...
def resolve_job_timeliness(session, date: str, bbox: list[float]) -> str:
    ...
    """
    scenes = fetch_s3_wfr_scenes(session, bbox, date)
...
def derived_from_uri(product_id: str) -> str:
    """
    creo://EODATA source path for an OLCI L2 product, matching the existing
    archive convention (zero-padded date folders, padded product type):
        creo://EODATA/Sentinel-3/OLCI/OL_2_WFR___/YYYY/MM/DD/{id}.SEN3
    """
    pid = product_id if product_id.endswith(".SEN3") else f"{product_id}.SEN3"
    padded_type = pid[4:15]  # "OL_2_WFR___"
    start_ts = pid[16:31]  # "20160407T111839"
    year, month, day = start_ts[0:4], start_ts[4:6], start_ts[6:8]
    return f"creo://EODATA/Sentinel-3/OLCI/{padded_type}/{year}/{month}/{day}/{pid}"


def build_s3_wfr_stac_item(
    tile: str,
    date: str,
    ...
    """
    Build a validated pystac.Item for one s3_wfr tile/date dataset.

    Follows the reference s3_olci_l2_wfr item: STAC 1.1.0, projection v2.0.0
    (proj:code + per-asset shape/transform in the tile's UTM CRS), eo (cloud
    cover averaged over source scenes), product (product:type),
    derived_from -> creo://EODATA source products. No timeliness properties
    (matching the reference, which omits them).
    """
    if not scenes:
    ...
    # Tile bounds in the native CRS: the exact values captured from the gpkg
    # layer before reprojection (load_tiling_grid always sets these).
    ...
    year, month, day = date.split("-")
    item_id = f"Sentinel-3_OLCI_{PRODUCT_TYPE}_{year}_{month}_{day}_{tile}"
    ...
    if covers:
        EOExtension.ext(item, add_if_missing=True).cloud_cover = round(
            statistics.mean(covers)
        )

    # Product extension (product:type only - the reference WFR item carries
    # no timeliness properties)
    ...
    for band, nodata in S3_WFR_ASSET_NODATA.items():
        item.add_asset(
            band,
            pystac.Asset(
...
def generate_stac_items(
    ...
    def process_tile(row) -> dict:
        ...
        try:
            scenes = fetch_s3_wfr_scenes(session, tile_bbox, date)
        except Exception as e:
            log.error(f"  scene lookup failed for {tile}: {e}")
            return {"tile": tile, "status": "error", "error": str(e)}

        if not scenes:
            log.info(f"  {tile}: no OL_2_WFR scenes on {date} - skipping")
        ...
        first_band = next(iter(S3_WFR_ASSET_NODATA))
        ...
        try:
            item = build_s3_wfr_stac_item(tile, date, grid, scenes, base_uri)
        except Exception as e:
...
@click.group("s3-wfr-cdse-pipeline")
def cli():
    """
    Run s3_wfr batch jobs (one per date) in three stages, each independently
...
def _submit_one(
    ...
    return submit_one(
        ...
        + "/<outputId>.<format>",
        description=f"Batch Sentinel-3 WFR {date}",
        aoi_bbox=aoi_bbox,
        dry_run=dry_run,
    )
...
def stac(date, end_date, output_bucket, href_bucket, full_aoi, timeliness, test):
    ...
    # --test: no job needed - build one item against the SH catalog and
    # print it, to eyeball the metadata before paying for a batch run.
    if test:
        ...
        for d in dates:
            for _, row in tiles.iterrows():
                ...
                scenes = fetch_s3_wfr_scenes(session, tile_bbox, d)
                if not scenes:
                    log.info(f"{tile}: no OL_2_WFR scenes on {d}")
                    continue
                tile_timeliness = timeliness or _timeliness(scenes[0]["id"])
                base_uri = deafrica_base_uri(d, tile, tile_timeliness, href_bucket)
                item = build_s3_wfr_stac_item(tile, d, grid, scenes, base_uri)
                if item:
                    out_path = save_stac_item(item)
