...
Three stages, each its own subcommand (so each can run as its own Argo step):
    ...
    s3-lfr-batch-test submit -d 2026-02-09   # create + analyse + start; prints
                                             # {"job_id": ..., "timeliness": ...}
    s3-lfr-batch-test wait --job-id <id>     # poll until terminal; exit 0 iff DONE
    s3-lfr-batch-test stac -d 2026-02-09     # build + upload STAC items
"""
...
def _submit_one(
    ...
    payload["description"] = f"Batch Sentinel-3 LFR test {date}"
