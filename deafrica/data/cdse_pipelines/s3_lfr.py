#!/usr/bin/env python3
"""
Submits a CDSE Batch Processing V2 job for Sentinel-3 OLCI Level 2 Land (s3_lfr).
...
Three stages, each its own subcommand (so each can run as its own Argo step):
    ...
    s3-lfr-cdse-pipeline submit -d 2026-02-09   # create + analyse + start; prints
                                                # {"jobs": [{date, job_id, timeliness, status}]}
    s3-lfr-cdse-pipeline wait --job-id <id>     # poll until terminal; exit 0 iff DONE
    s3-lfr-cdse-pipeline stac -d 2026-02-09     # build + upload STAC items
 
End-to-end example:
 
    DATE=2026-02-09
    OUT=$(s3-lfr-cdse-pipeline submit -d "$DATE" --full-aoi)
    JOB_ID=$(jq -r '.jobs[0].job_id' <<< "$OUT")
    TIMELINESS=$(jq -r '.jobs[0].timeliness' <<< "$OUT")
    s3-lfr-cdse-pipeline wait --job-id "$JOB_ID"
    s3-lfr-cdse-pipeline stac -d "$DATE" --timeliness "$TIMELINESS"
"""
...
ODC_PRODUCT = "s3_olci_l2_lfr"

# Output bands written by evalscripts/s3_lfr.js and their nodata values
S3_LFR_ASSET_NODATA = {
    "GIFAPAR": 65535,
    "IWV_L": 255,
    "OTCI": 65535,
...
def deafrica_base_uri(
    ...
    The DE Africa prefix for a dataset:
        ...
        s3://{bucket}/Sentinel-3/OLCI/OL_2_LFR/{YYYY}/{MM}/
            S3_OL_2_LFR_{YYYYMMDD}_{timeliness}_{tile}
    """
...
def _fetch_grid_from_cloudferro(grid_href: str) -> str:
    ...
    local_path = os.path.join(
        tempfile.gettempdir(), f"s3_lfr_grid_{os.path.basename(key)}"
    )
...
def load_tiling_grid(sat_config: dict) -> gpd.GeoDataFrame:
    """
    Load the s3_lfr tiling geopackage into a single EPSG:4326 GeoDataFrame.
    ...
    log.info(f"Loading s3_lfr tiling grid from {grid_href} ...")
...
def _acquisition_key(product_id: str) -> tuple[str, str]:
    ...
    S3A_OL_2_LFR____20160407T111839_...  ->  ("S3A", "20160407T111839").
    """
    return product_id[:3], product_id[16:31]


def fetch_s3_lfr_scenes(session, bbox: list[float], date: str) -> list[dict]:
    """
    Query the SH catalog for all OL_2_LFR scenes
...
def derived_from_uri(product_id: str) -> str:
    ...
    archive convention (zero-padded date folders, padded product type):
        creo://EODATA/Sentinel-3/OLCI/OL_2_LFR___/YYYY/MM/DD/{id}.SEN3
    """
    pid = product_id if product_id.endswith(".SEN3") else f"{product_id}.SEN3"
    padded_type = pid[4:15]  # "OL_2_LFR___"
...
def build_s3_lfr_stac_item(
    ...
    """
    Build a validated pystac.Item for one s3_lfr tile/date dataset.
    ...
    primary_id = scenes[0]["id"].removesuffix(".SEN3").removesuffix("_COG")
    ...
    timeliness_cat = _timeliness(primary_id)
    ...
    for band, nodata in S3_LFR_ASSET_NODATA.items():
        item.add_asset(
            band,
            pystac.Asset(
...
def generate_stac_items(
    ...
    def process_tile(row) -> dict:
        ...
        try:
            scenes = fetch_s3_lfr_scenes(session, tile_bbox, date)
        except Exception as e:
            log.error(f"  scene lookup failed for {tile}: {e}")
            return {"tile": tile, "status": "error", "error": str(e)}

        if not scenes:
            log.info(f"  {tile}: no OL_2_LFR scenes on {date} - skipping")
        ...
        first_band = next(iter(S3_LFR_ASSET_NODATA))
        ...
        try:
            item = build_s3_lfr_stac_item(tile, date, grid, scenes, base_uri)
        except Exception as e:
    ...
    if test:
        ...
        for d in dates:
            for _, row in tiles.iterrows():
                ...
                scenes = fetch_s3_lfr_scenes(session, tile_bbox, d)
                if not scenes:
                    log.info(f"{tile}: no OL_2_LFR scenes on {d}")
                    continue
                tile_timeliness = timeliness or _timeliness(scenes[0]["id"])
                base_uri = deafrica_base_uri(d, tile, tile_timeliness, href_bucket)
                item = build_s3_lfr_stac_item(tile, d, grid, scenes, base_uri)
                if item:
                    out_path = save_stac_item(item)
