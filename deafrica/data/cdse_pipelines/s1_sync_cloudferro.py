...
can sync a single CDSE tile output prefix, or discover completed prefixes under
a broader CloudFerro prefix and sync them with bounded parallelism.
...
DEFAULT_DISCOVERY_PREFIX = "s1_rtc/"
...
def sync_all_prefixes(
    ...
    discovery_prefix = normalize_s3_prefix(discovery_prefix)
    max_workers = max(1, max_workers)

    log.info(
        "Discovering completed CloudFerro prefixes under: s3://%s/%s",
        config.source_bucket,
        discovery_prefix,
    )
    ...
    completed_prefixes = discovery["completed_prefixes"]
    ...
    summary = {
        ...
        "discovered_prefixes": len(completed_prefixes)
        + len(discovery["skipped_prefixes"]),
        "completed_prefixes": len(completed_prefixes),
...
def discover_completed_prefixes(client, bucket: str, discovery_prefix: str) -> dict:
    """Find completed Sentinel-1 tile prefixes under a broader source prefix."""
    ...
    for source_object in list_source_objects(client, bucket, discovery_prefix):
        source_key = source_object["Key"]
        source_prefix = tile_prefix_from_key(source_key)
    ...
    for source_prefix, source_objects in sorted(objects_by_prefix.items()):
        ...
        stac_item = find_stac_item(source_objects)
        if not stac_item:
            skipped_prefixes.append(
                {
                    "source_prefix": source_prefix,
...
def tile_prefix_from_key(source_key: str) -> str | None:
    try:
...
def find_stac_item(source_objects: list[dict]) -> str | None:
    ...
    for source_object in sorted(source_objects, key=lambda item: item["Key"]):
        ...
        if filename.endswith("_metadata.json") and skip_reason(source_key) is None:
            return source_key
...
@click.option(
    ...
    help="CloudFerro prefix to scan when --all-prefixes is set.",
)
@click.option(
    "--max-workers",
...
def cli(
    ...
    """Sync Sentinel-1 CDSE tile outputs from CloudFerro to AWS S3."""
    if all_prefixes:
        if job_id:
        ...
        if not stac_item:
            raise click.UsageError(
                "--stac-item is required unless --all-prefixes is set"
            )
