#!/usr/bin/env python3
"""
Submits CDSE Batch Processing V2 jobs for the Sentinel-5P TROPOMI L2
products (aer_ai, ch4, cloud, co, hcho, no2, o3, so2). All products share
identical pipeline logic; per-product config lives in S5P_PRODUCTS and is
selected with --product / -p.
...
Three stages, each its own subcommand (so each can run as its own Argo step):
    ...
    s5p-cdse-pipeline submit -p no2 -d 2026-02-09   # create + analyse + start; prints
                                                    # {"job_id": ..., "timeliness": ...}
    s5p-cdse-pipeline wait --job-id <id>            # poll until terminal; exit 0 iff DONE
    s5p-cdse-pipeline stac -p no2 -d 2026-02-09     # build + upload STAC items
"""
...
# Per-product configuration. For each product:
#   native_type:  padded product type as it appears in S5P product ids and
#                 EODATA paths (always 10 chars, = product_id[9:19])
#   product_type: DE Africa product type used in item ids, folder names,
#                 and product:type
#   odc_product:  odc:product value
#   bucket:       final DE Africa archive bucket (default --href-bucket)
#   asset_nodata: output bands written by the product's evalscript and
#                 their nodata values, in metadata.json asset order
S5P_PRODUCTS = {
    "aer_ai": {
        "native_type": "L2__AER_AI",
        "product_type": "TROPO_L2_AI",
        "odc_product": "s5p_tropomi_l2_aer_ai",
        "bucket": "deafrica-sentinel-5p-aer-ai",
        "asset_nodata": {
            "AER_AI_340_380": -9999,
            "AER_AI_354_388": -9999,
            "dataMask": 255,
    ...
    "ch4": {
        "native_type": "L2__CH4___",
        "product_type": "TROPO_L2_CH4",
        "odc_product": "s5p_tropomi_l2_ch4",
        "bucket": "deafrica-sentinel-5p-ch4",
        "asset_nodata": {"CH4": -9999, "dataMask": 255},
    },
    "cloud": {
        "native_type": "L2__CLOUD_",
        "product_type": "TROPO_L2_CLOUD",
        "odc_product": "s5p_tropomi_l2_cloud",
        "bucket": "deafrica-sentinel-5p-cloud",
        "asset_nodata": {
            "CLOUD_BASE_PRESSURE": -9999,
            "CLOUD_TOP_PRESSURE": -9999,
            "CLOUD_BASE_HEIGHT": -9999,
            "CLOUD_TOP_HEIGHT": -9999,
            "CLOUD_OPTICAL_THICKNESS": -9999,
            "CLOUD_FRACTION": -9999,
            "dataMask": 255,
        },
    },
    "co": {
        "native_type": "L2__CO____",
        "product_type": "TROPO_L2_CO",
        "odc_product": "s5p_tropomi_l2_co",
        "bucket": "deafrica-sentinel-5p-co",
        "asset_nodata": {"CO": -9999, "dataMask": 255},
    },
    "hcho": {
        "native_type": "L2__HCHO__",
        "product_type": "TROPO_L2_HCHO",
        "odc_product": "s5p_tropomi_l2_hcho",
        "bucket": "deafrica-sentinel-5p-hcho",
        "asset_nodata": {"HCHO": -9999, "dataMask": 255},
    },
    "no2": {
        "native_type": "L2__NO2___",
        "product_type": "TROPO_L2_NO2",
        "odc_product": "s5p_tropomi_l2_no2",
        "bucket": "deafrica-sentinel-5p-no2",
        "asset_nodata": {"NO2": -9999, "dataMask": 255},
    },
    "o3": {
        "native_type": "L2__O3____",
        "product_type": "TROPO_L2_O3",
        "odc_product": "s5p_tropomi_l2_o3",
        "bucket": "deafrica-sentinel-5p-o3",
        "asset_nodata": {"O3": -9999, "dataMask": 255},
    },
    "so2": {
        "native_type": "L2__SO2___",
        "product_type": "TROPO_L2_SO2",
        "odc_product": "s5p_tropomi_l2_so2",
        "bucket": "deafrica-sentinel-5p-so2",
        "asset_nodata": {"SO2": -9999, "dataMask": 255},
    },
}
...
def deafrica_base_uri(
    ...
    """
    The DE Africa prefix for a dataset (bucket defaults to the product's
    final archive bucket from S5P_PRODUCTS):

        s3://{bucket}/Sentinel-5p/TROPOMI/{product_type}/{YYYY}/{MM}/
            S5p_{product_type}_{YYYYMMDD}_{timeliness}_{tile}
    """
    bucket = bucket or cfg["bucket"]
    ptype = cfg["product_type"]
    year, month, day = date.split("-")
    dataset = f"S5p_{ptype}_{year}{month}{day}_{timeliness}_{tile}"
    return f"s3://{bucket}/Sentinel-5p/TROPOMI/{ptype}/{year}/{month}/{dataset}"
...
def _fetch_grid_from_cloudferro(grid_href: str) -> str:
    ...
    local_path = os.path.join(
        tempfile.gettempdir(), f"s5p_grid_{os.path.basename(key)}"
    )
...
def fetch_s5p_scenes(session, cfg: dict, bbox: list[float], date: str) -> list[dict]:
    """
    Query the SH catalog for all of the product's scenes
    ...
    while True:
        ...
        data = resp.json()
        features.extend(
            f for f in data.get("features", []) if cfg["native_type"] in f["id"]
        )
...
def resolve_job_timeliness(session, cfg: dict, date: str, bbox: list[float]) -> str:
    """
    ...
    """
    scenes = fetch_s5p_scenes(session, cfg, bbox, date)
...
def build_s5p_stac_item(
    ...
    """
    Build a validated pystac.Item for one tile/date dataset of the product.
    ...
    year, month, day = date.split("-")
    item_id = f"Sentinel-5p_TROPOMI_{cfg['product_type']}_{year}_{month}_{day}_{tile}"
    ...
    item = pystac.Item(
        ...
        properties={
            "title": item_id,
            "description": f"Sentinel-5p product {cfg['product_type']} for {tile}",
            "platform": "Sentinel-5p",
            "instruments": ["TROPOMI"],
            # stripped ("33QZA_0_0" -> "33QZA")
            "odc:region_code": tile.split("_")[0],
            "odc:product": cfg["odc_product"],
            "license": LICENSE,
    ...
    item.properties["product:type"] = cfg["product_type"]
    ...
    for band, nodata in cfg["asset_nodata"].items():
        item.add_asset(
            band,
            pystac.Asset(
...
def generate_stac_items(
    ...
    def process_tile(row) -> dict:
        ...
        staging = deafrica_base_uri(cfg, date, tile, job_timeliness, output_bucket)
        ...
        try:
            scenes = fetch_s5p_scenes(session, cfg, tile_bbox, date)
        ...
        first_band = next(iter(cfg["asset_nodata"]))
        ...
        base_uri = deafrica_base_uri(cfg, date, tile, job_timeliness, href_bucket)

        try:
            item = build_s5p_stac_item(cfg, tile, date, grid, scenes, base_uri)
        except Exception as e:
...
_href_bucket_option = click.option(
    ...
    "Defaults to the product's bucket from S5P_PRODUCTS. Only affects the "
    "URIs inside metadata.json, not where anything is physically written.",
)
...
_product_option = click.option(
    "--product",
    "-p",
    required=True,
    type=click.Choice(sorted(S5P_PRODUCTS)),
    help="Sentinel-5P product to process (selects config from S5P_PRODUCTS "
    "and SATELLITES['s5p_<product>']).",
)


@click.group("s5p-cdse-pipeline")
def cli():
    """
    Run Sentinel-5P batch jobs (one per product per date) in three stages,
    each independently runnable (and retryable) as its own Argo step:
...
def _submit_one(
    session, cfg, sat_config, grid, date, output_bucket, aoi_bbox, dry_run
) -> dict:
    """
    ...
    job_timeliness = resolve_job_timeliness(session, cfg, date, timeliness_bbox)
    log.info(
    ...
    payload["description"] = f"Batch Sentinel-5p {cfg['product_type']} test {date}"

    # Physical delivery to CF bucket (--output-bucket)
    payload["output"]["delivery"]["s3"]["url"] = (
        deafrica_base_uri(cfg, date, "<tileName>", job_timeliness, output_bucket)
        + "/<outputId>.<format>"
    )
    ...
    log.info(
        ...
        f"s3://{output_bucket}/Sentinel-5p/TROPOMI/{cfg['product_type']}/"
        f"{year}/{month}/ - same keys as the final archive"
    )
...
def submit(product, date, end_date, output_bucket, full_aoi, dry_run):
    """
    ...
    """
    cfg = S5P_PRODUCTS[product]
    sat_config = SATELLITES[f"s5p_{product}"]
    session = get_session()
    ...
    for d in _date_range(date, end_date):
        try:
            jobs.append(
                _submit_one(
                    session, cfg, sat_config, grid, d, output_bucket, aoi_bbox, dry_run
                )
            )
        except Exception as e:
...
def stac(product, date, end_date, output_bucket, href_bucket, full_aoi, timeliness, max_workers, test):
    ...
    """
    cfg = S5P_PRODUCTS[product]
    href_bucket = href_bucket or cfg["bucket"]
    sat_config = SATELLITES[f"s5p_{product}"]
    session = get_session()
    ...
    if test:
        ...
        for d in dates:
            for _, row in tiles.iterrows():
                ...
                scenes = fetch_s5p_scenes(session, cfg, tile_bbox, d)
                if not scenes:
                    log.info(f"{tile}: no {cfg['native_type']} scenes on {d}")
                    continue
                tile_timeliness = timeliness or _timeliness(scenes[0]["id"])
                base_uri = deafrica_base_uri(cfg, d, tile, tile_timeliness, href_bucket)
                item = build_s5p_stac_item(cfg, tile, d, grid, scenes, base_uri)
                if item:
                    out_path = save_stac_item(item)
    ...
    for d in dates:
        if timeliness:
            ...
            timeliness_bbox = aoi_bbox or [round(v, 4) for v in grid.total_bounds]
            job_timeliness = resolve_job_timeliness(session, cfg, d, timeliness_bbox)
        log.info(
