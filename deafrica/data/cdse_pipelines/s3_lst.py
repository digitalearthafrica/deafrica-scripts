#!/usr/bin/env python3
"""
Submits a CDSE Batch Processing V2 job for Sentinel-3 SLSTR Level 2 Land Surface Temperature (s3_lst).
...
Three stages, each its own subcommand (so each can run as its own Argo step):
    ...
    s3-lst-batch-test submit -d 2026-02-09   # create + analyse + start; prints
                                             # {"job_id": ..., "timeliness": ...}
    s3-lst-batch-test wait --job-id <id>     # poll until terminal; exit 0 iff DONE
    s3-lst-batch-test stac -d 2026-02-09     # build + upload STAC items
"""
...
S3_SLSTR_L2_COLLECTION = "sentinel-3-slstr-l2"
PRODUCT_TYPE = "SL_2_LST"
ODC_PRODUCT = "s3_slstr_l2_lst"

# Output bands written by evalscripts/s3_lst.js and their nodata values
S3_LST_ASSET_NODATA = {
    "LST": 65535,
    "LST_uncertainty": 65535,
    "NDVI": 32767,
    "CONFIDENCE": 65535,
    "dataMask": 255,
}

# product:timeliness (ISO 8601 duration) per Copernicus timeliness category:
# NR (near-real-time) <= 3h, ST (short-time-critical) <= 48h, NT (non-time-
# critical) <= 1 month.
TIMELINESS_DURATION = {"NR": "PT3H", "ST": "PT48H", "NT": "P1M"}
...
EXTENSION_ORDER = ("/product/", "/projection/", "/eo/")

# Final home of the products - the bucket all STAC hrefs point at by default (--href-bucket)
DEAFRICA_S3_BUCKET = "deafrica-sentinel-3-slstr-l2-lst"
...
def deafrica_base_uri(
    ...
    The DE Africa prefix for a dataset:
        ...
        s3://{bucket}/Sentinel-3/SLSTR/SL_2_LST/{YYYY}/{MM}/
            S3_SL_2_LST_{YYYYMMDD}_{timeliness}_{tile}
    """
    year, month, day = date.split("-")
    dataset = f"S3_{PRODUCT_TYPE}_{year}{month}{day}_{timeliness}_{tile}"
    return f"s3://{bucket}/Sentinel-3/SLSTR/{PRODUCT_TYPE}/{year}/{month}/{dataset}"
...
def _fetch_grid_from_cloudferro(grid_href: str) -> str:
    ...
    local_path = os.path.join(
        tempfile.gettempdir(), f"s3_lst_grid_{os.path.basename(key)}"
    )
...
def load_tiling_grid(sat_config: dict) -> gpd.GeoDataFrame:
    """
    Load the s3_lst tiling geopackage into a single EPSG:4326 GeoDataFrame.
    ...
    log.info(f"Loading s3_lst tiling grid from {grid_href} ...")
...
def epsg_from_tile_name(tile: str) -> int:
    ...
    e.g. "28RCP_0_0" -> zone 28, latitude band R (northern) -> EPSG:32628.
...
def _timeliness(product_id: str) -> str:
    "NT / NR / ST timeliness token, second-to-last '_' field of the id."
    clean = product_id.removesuffix(".SEN3")
    return clean.rsplit("_", 2)[1]


def _baseline(product_id: str) -> str:
    "Processing baseline collection, last '_' field of the id (e.g. '003')."
    return product_id.removesuffix(".SEN3").rsplit("_", 2)[2]


def _acquisition_key(product_id: str) -> tuple[str, str]:
    """
    Key identifying a unique acquisition regardless of timeliness/baseline:
    (platform, sensing start timestamp). Product ids have fixed-width fields:
    S3A_SL_2_LST____20160407T111839_...  ->  ("S3A", "20160407T111839").
    """
    return product_id[:3], product_id[16:31]


def fetch_s3_lst_scenes(session, bbox: list[float], date: str) -> list[dict]:
    """
    Query the SH catalog for all SL_2_LST scenes
    intersecting bbox on the given date. NR/NT duplicates of the same
    acquisition are deduped preferring NTC, matching
    the batch's default "mostRecent" mosaicking behaviour where NTC (Non-Time Critical) is
    preferred over NRT (Near Real-Time).
    """
    body = {
        "collections": [S3_SLSTR_L2_COLLECTION],
        "bbox": [round(v, 4) for v in bbox],
        "datetime": f"{date}T00:00:00Z/{date}T23:59:59Z",
        "limit": 100,
        # No fields filter - we want full properties (cloud cover, datetimes)
    }
    ...
    # Dedupe NR/NT versions of the same acquisition: prefer NTC, then baseline
    ...
    for f in features:
        ...
        cur_rank = (_timeliness(cur) == "NT", _baseline(cur))
        new_rank = (_timeliness(new) == "NT", _baseline(new))
...
def resolve_job_timeliness(session, date: str, bbox: list[float]) -> str:
    ...
    if their timeliness is uniform, use it; if mixed (e.g. mid rolling
    reprocessing), prefer NTC - matching both the dedup preference and the
    batch's mostRecent mosaicking - and warn.
    """
    scenes = fetch_s3_lst_scenes(session, bbox, date)
    ...
    if len(cats) > 1:
        ...
        log.warning(
            ...
            f"delivery folder names (per-item product:timeliness_category "
            f"still reflects each tile's actual scene)"
        )
        return chosen
    return cats.pop()
...
def derived_from_uri(product_id: str) -> str:
    """
    creo://eodata source path for an SLSTR L2 product, matching the current
    archive convention (lowercase eodata, zero-padded date folders, padded
    product type):
        creo://eodata/Sentinel-3/SLSTR/SL_2_LST___/YYYY/MM/DD/{id}.SEN3
    """
    pid = product_id if product_id.endswith(".SEN3") else f"{product_id}.SEN3"
    padded_type = pid[4:15]  # "SL_2_LST___"
    start_ts = pid[16:31]  # "20260505T185742"
    year, month, day = start_ts[0:4], start_ts[4:6], start_ts[6:8]
    return f"creo://eodata/Sentinel-3/SLSTR/{padded_type}/{year}/{month}/{day}/{pid}"


def build_s3_lst_stac_item(
    tile: str,
    date: str,
    ...
    """
    Build a validated pystac.Item for one s3_lst tile/date dataset.
    ...
    # Item geometry/bbox: the tile footprint clipped to the union of the
    # contributing scenes' footprints, so partially covered tiles trace the
    # swath edge (matching production items) instead of claiming the full
    # tile. Falls back to the full tile when the scenes fully cover it or
    # carry no geometry.
    tile_poly = Polygon(ring)
    footprint = tile_poly
    scene_geoms = [shape(s["geometry"]) for s in scenes if s.get("geometry")]
    if scene_geoms:
        clipped = tile_poly.intersection(unary_union(scene_geoms))
        if not clipped.is_empty and clipped.area > 0:
            footprint = clipped

    def _r15_coords(coords):
        if isinstance(coords[0], (int, float)):
            return [_r15(c) for c in coords]
        return [_r15_coords(c) for c in coords]

    geom = mapping(footprint)
    item_geometry = {
        "type": geom["type"],
        "coordinates": _r15_coords([list(c) for c in geom["coordinates"]]),
    }
    fminx, fminy, fmaxx, fmaxy = footprint.bounds
    item_bbox = [_r15(fminx), _r15(fminy), _r15(fmaxx), _r15(fmaxy)]
    ...
    year, month, day = date.split("-")
    item_id = f"Sentinel-3_SLSTR_{PRODUCT_TYPE}_{year}_{month}_{day}_{tile}"
    ...
    item = pystac.Item(
        ...
        properties={
            "title": item_id,
            "description": f"Sentinel-3 product {PRODUCT_TYPE} for {tile}",
            "platform": "Sentinel-3",
            "instruments": ["SLSTR"],
            # stripped ("37NBB_0_0" -> "37NBB")
    ...
    if start_dt:
        ...
        item.properties["end_datetime"] = end_dt.isoformat(
            timespec="milliseconds"
        ).replace("+00:00", "Z")
    ...
    # Cloud cover averaged over the contributing scenes
    covers = [
        s["properties"]["eo:cloud_cover"]
        for s in scenes
        if s["properties"].get("eo:cloud_cover") is not None
    ]
    if covers:
        # Production LST items keep decimal precision (e.g. 0.06), unlike
        # the OLCI convention of rounding to an integer.
        EOExtension.ext(item, add_if_missing=True).cloud_cover = round(
            statistics.mean(covers), 2
        )

    # Product extension. product:timeliness_category reflects the tile's
    # actual scene, which can differ from the delivery folder token (e.g. an
    # NR scene inside an _NT_ folder on a mixed-timeliness day).
    ...
    timeliness_cat = _timeliness(scenes[0]["id"].removesuffix(".SEN3"))
    if timeliness_cat in TIMELINESS_DURATION:
    ...
    for band, nodata in S3_LST_ASSET_NODATA.items():
        item.add_asset(
            band,
            pystac.Asset(
...
def generate_stac_items(
    ...
    def process_tile(row) -> dict:
        ...
        try:
            scenes = fetch_s3_lst_scenes(session, tile_bbox, date)
        except Exception as e:
            log.error(f"  scene lookup failed for {tile}: {e}")
            return {"tile": tile, "status": "error", "error": str(e)}

        if not scenes:
            log.info(f"  {tile}: no SL_2_LST scenes on {date} - skipping")
        ...
        first_band = next(iter(S3_LST_ASSET_NODATA))
        ...
        try:
            item = build_s3_lst_stac_item(tile, date, grid, scenes, base_uri)
        except Exception as e:
...
@click.group("s3-lst-cdse-pipeline")
def cli():
    """
    Run s3_lst batch jobs (one per date) in three stages, each independently
    runnable (and retryable) as its own Argo step:
...
def _submit_one(
    ...
    payload["description"] = f"Batch Sentinel-3 LST test {date}"
    ...
    log.info(
        ...
        f"s3://{output_bucket}/Sentinel-3/SLSTR/{PRODUCT_TYPE}/{year}/{month}/ "
...
@click.option(
    "--timeliness",
    default=None,
    type=click.Choice(sorted(TIMELINESS_DURATION)),
    ...
    "with the delivered folders if scenes changed (e.g. NR superseded by "
    "NT) since submission.",
)
@click.option(
    "--max-workers",
    default=8,
    show_default=True,
    help="Parallel worker threads for STAC generation. The work is I/O "
    "bound (catalog queries + S3 head/put), so 16-32 is safe for "
    "full-AOI runs.",
)
@click.option(
    "--test",
    ...
    if test:
        ...
        for d in dates:
            for _, row in tiles.iterrows():
                ...
                scenes = fetch_s3_lst_scenes(session, tile_bbox, d)
                if not scenes:
                    log.info(f"{tile}: no SL_2_LST scenes on {d}")
                    continue
                tile_timeliness = timeliness or _timeliness(scenes[0]["id"])
                base_uri = deafrica_base_uri(d, tile, tile_timeliness, href_bucket)
                item = build_s3_lst_stac_item(tile, d, grid, scenes, base_uri)
                if item:
                    out_path = save_stac_item(item)
