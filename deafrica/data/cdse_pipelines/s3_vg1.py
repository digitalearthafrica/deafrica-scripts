...
# Output bands written by evalscripts/s3_vg1.js and their nodata values
# (order matches the production metadata.json asset order; note VG1 items
# have no dataMask asset)
S3_VG1_ASSET_NODATA = {
    "B0_VG1": 32767,
    "B2_VG1": 32767,
    "B3_VG1": 32767,
...
DEAFRICA_S3_BUCKET = "deafrica-sentinel-3-synergy-vegetation"
...
def deafrica_base_uri(
    ...
    The DE Africa prefix for a dataset:
        ...
        s3://{bucket}/Sentinel-3/SYN/SY_2_VG1/{YYYY}/{MM}/
            S3_SY_2_VG1_{YYYYMMDD}_{timeliness}_{tile}
    """
    year, month, day = date.split("-")
    dataset = f"S3_{PRODUCT_TYPE}_{year}{month}{day}_{timeliness}_{tile}"
    return f"s3://{bucket}/Sentinel-3/SYN/{PRODUCT_TYPE}/{year}/{month}/{dataset}"
...
def _fetch_grid_from_cloudferro(grid_href: str) -> str:
    ...
    local_path = os.path.join(
        tempfile.gettempdir(), f"s3_vg1_grid_{os.path.basename(key)}"
    )
...
def load_tiling_grid(sat_config: dict) -> gpd.GeoDataFrame:
    """
    Load the s3_vg1 tiling geopackage into a single EPSG:4326 GeoDataFrame.
    ...
    log.info(f"Loading s3_vg1 tiling grid from {grid_href} ...")
...
def _acquisition_key(product_id: str) -> tuple[str, str]:
    ...
    S3A_SY_2_VG1____20160407T111839_...  ->  ("S3A", "20160407T111839").
    """
    return product_id[:3], product_id[16:31]


def fetch_s3_vg1_scenes(session, bbox: list[float], date: str) -> list[dict]:
    """
    Query the SH catalog for all SY_2_VG1 scenes
    ...
    """
    body = {
        "collections": [S3_SYN_L2_COLLECTION],
...
def resolve_job_timeliness(session, date: str, bbox: list[float]) -> str:
    ...
    """
    scenes = fetch_s3_vg1_scenes(session, bbox, date)
...
def build_s3_vg1_stac_item(
    ...
    """
    Build a validated pystac.Item for one s3_vg1 tile/date dataset.
    ...
    year, month, day = date.split("-")
    item_id = f"Sentinel-3_SYN_{PRODUCT_TYPE}_{year}_{month}_{day}_{tile}"
    ...
    if covers:
        # Production items keep decimal precision (e.g. 0.06), unlike
    ...
    # Product extension (production VG1 items carry product:type only - no
    # product:timeliness / product:timeliness_category)
    ...
    for band, nodata in S3_VG1_ASSET_NODATA.items():
        item.add_asset(
            band,
            pystac.Asset(
...
def generate_stac_items(
    ...
    def process_tile(row) -> dict:
        ...
        try:
            scenes = fetch_s3_vg1_scenes(session, tile_bbox, date)
        except Exception as e:
            log.error(f"  scene lookup failed for {tile}: {e}")
            return {"tile": tile, "status": "error", "error": str(e)}

        if not scenes:
            log.info(f"  {tile}: no SY_2_VG1 scenes on {date} - skipping")
        ...
        first_band = next(iter(S3_VG1_ASSET_NODATA))
        ...
        try:
            item = build_s3_vg1_stac_item(tile, date, grid, scenes, base_uri)
        except Exception as e:
...
@click.group("s3-vg1-cdse-pipeline")
def cli():
    """
    Run s3_vg1 batch jobs (one per date) in three stages, each independently
    ...
    if test:
        ...
        for d in dates:
            for _, row in tiles.iterrows():
                ...
                scenes = fetch_s3_vg1_scenes(session, tile_bbox, d)
                if not scenes:
                    log.info(f"{tile}: no SY_2_VG1 scenes on {d}")
                    continue
                tile_timeliness = timeliness or _timeliness(scenes[0]["id"])
                base_uri = deafrica_base_uri(d, tile, tile_timeliness, href_bucket)
                item = build_s3_vg1_stac_item(tile, d, grid, scenes, base_uri)
                if item:
                    out_path = save_stac_item(item)
