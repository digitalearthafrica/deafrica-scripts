...
SOURCE_URL_PATH = URL("https://zenodo.org/records/6894273/files/gmw_v3_2016_gtiff.zip?download=1")
FILE_NAME = "GMW_{year}"
...
def create_and_upload_stac(cog_file: Path, s3_dst: str, year) -> Item:
    ...
    item = create_stac_item(
        str(cog_file),
        id=str(odc_uuid("gmw", "3.0", [cog_file.stem])),
        with_proj=True,
...
def gmw_download_stac_cog(year: str, s3_dst: str, slack_url: str = None) -> None:
    ...
    try:
        ...
        extracted_tif = download_and_unzip_gmw(year)

        output_file = LOCAL_DIR / extracted_tif
        ...
        message = f"Failed to handle GMW data for year {year} with error {e}"
