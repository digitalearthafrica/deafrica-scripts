...
AWS_SECRET_ACCESS_KEY = os.getenv('AWS_SECRET_ACCESS_KEY')  # Secret key from environment
...
SENDER_PASSWORD = os.getenv('SENDER_PASSWORD')  # Get email password from environment variable
RECEIVER_EMAIL = os.getenv('RECEIVER_EMAIL')  # Get receiver email from environment variable
SMTP_SERVER = os.getenv('SMTP_SERVER', 'smtp.gmail.com')  # Default to Gmail if not provided
...
def convert_json_to_csv(json_filename, csv_filename):
    ...
    with open(csv_filename, mode='w', newline='') as csv_file:
        ...
        writer.writerow(['Name', 'Email', 'Email Verified', 'Phone Number Verified', 'MFA Enabled', 'Username', 'User Status', 'Enabled', 'User Create Date', 'User Last Modified Date'])  # Add more headers as needed
        ...
        for user in data['Users']:
            ...
            phone_number_verified = next((attr['Value'] for attr in user['Attributes'] if attr['Name'] == 'phone_number_verified'), 'FALSE')
            mfa_enabled = 'FALSE'
            ...
            writer.writerow([name, email, email_verified, phone_number_verified, mfa_enabled, username, user_status, enabled, user_create_date, user_last_modified_date])
